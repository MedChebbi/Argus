...
class ColorDetector:
    ...
    def __init__(self, params):
        self.params = params
        self.color_space = self.params['color_space']
        self.min_values = np.array(self.params['color_min_range'])
        self.max_values = np.array(self.params['color_max_range'])
        self.max_area_thre = self.params["MAX_AREA"]
    ...
    def fine_tune(self):
        self.min_values, self.max_values = self._extract_values()
    ...
    def detect(self, frame, debug_frame, params=None, draw = False):
        ...
        if contours_len > 0 and (self.min_area_thre < max_area < self.max_area_thre):
            ...
            if self.params['detection_mode'] == "line":
                box = self._detect_black(contours, setpoint)
    ...
    def _detect_black(self, contours, setpoint):
        coff = 10
        contours_len = len(contours)
        if contours_len == 1:
            x,y,w,h = cv2.boundingRect(contours[0])
